...
def tf_image_loader(out_size, 
    ...
    def _func(X):
        with tf.compat.v1.name_scope('image_augmentation'):
            ...
            with tf.compat.v1.name_scope('augmentation'):
                if horizontal_flip:
...
def tf_augmentor(out_size,
    ...
    def batch_ops(X, y):
        ...
        with tf.compat.v1.name_scope('transformation'):
            ...
            if crop_probability > 0:
                ...
                transforms += [tf.compat.v1.where(coin, crop_transform, tf.tile(tf.expand_dims(identity, 0), [batch_size, 1]))]
            if len(transforms)>0:
