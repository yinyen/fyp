...
from active_learning.dual_model_pipeline import TrainPipeline
from pytorch.dual_data_helper import create_data_loader
...
class DualRandomActiveLearning():
    def __init__(self, **config):
        ...
        test_df = create_dual_label_df(main_data_dir = "../all_train_300", train_dir_list = ["val"]).sample(2000, random_state = 123)
        full_df["ui"] = 0
        full_df["features"] = 0
    ...
    def extract_features_and_form_clusters(self, model, label_df, size, workers, **kwargs):
        data_loader = create_data_loader(label_df, size, batch_size = 2, workers = workers)
        f, y = extract_dual_features(model, data_loader)
        label_df["features"] = [j for j in f]

        centroid = {}
        for i in range(5):
            uf = label_df["labels_x"] == str(i)
    ...
    def extract_features_and_compute_index(self, model, unlabel_df, centroid, n, size, workers, outlier = 0, **kwargs):
        data_loader = create_data_loader(unlabel_df, size, batch_size = 2, workers = workers)
        f, y = extract_dual_features(model, data_loader)
    ...
    def evaluate(self, model, metric_fc, val_df, size, workers, metric_type, **kwargs):
        unseen_test_loader = create_data_loader(val_df, size, batch_size = 2, workers = workers)
        y_true = []
        y_pred = []
